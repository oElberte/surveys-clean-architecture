// Mocks generated by Mockito 5.2.0 from annotations
// in surveys/test/main/composites/remote_load_survey_result_with_local_fallback_test.dart.
// Do not manually edit this file.

import 'dart:async' as _i6;

import 'package:mockito/mockito.dart' as _i1;
import 'package:surveys/data/cache/cache.dart' as _i2;
import 'package:surveys/data/http/http.dart' as _i4;
import 'package:surveys/data/usecases/load_survey_result/local_load_survey_result.dart'
    as _i5;
import 'package:surveys/data/usecases/load_survey_result/remote_load_survey_result.dart'
    as _i7;
import 'package:surveys/domain/entities/entities.dart' as _i3;

// ignore_for_file: type=lint
// ignore_for_file: avoid_redundant_argument_values
// ignore_for_file: avoid_setters_without_getters
// ignore_for_file: comment_references
// ignore_for_file: implementation_imports
// ignore_for_file: invalid_use_of_visible_for_testing_member
// ignore_for_file: prefer_const_constructors
// ignore_for_file: unnecessary_parenthesis
// ignore_for_file: camel_case_types

class _FakeCacheStorage_0 extends _i1.Fake implements _i2.CacheStorage {}

class _FakeSurveyResultEntity_1 extends _i1.Fake
    implements _i3.SurveyResultEntity {}

class _FakeHttpClient_2 extends _i1.Fake implements _i4.HttpClient {}

/// A class which mocks [LocalLoadSurveyResult].
///
/// See the documentation for Mockito's code generation for more information.
class MockLocalLoadSurveyResult extends _i1.Mock
    implements _i5.LocalLoadSurveyResult {
  MockLocalLoadSurveyResult() {
    _i1.throwOnMissingStub(this);
  }

  @override
  _i2.CacheStorage get cacheStorage =>
      (super.noSuchMethod(Invocation.getter(#cacheStorage),
          returnValue: _FakeCacheStorage_0()) as _i2.CacheStorage);
  @override
  _i6.Future<_i3.SurveyResultEntity> loadBySurvey({String? surveyId}) => (super
      .noSuchMethod(Invocation.method(#loadBySurvey, [], {#surveyId: surveyId}),
          returnValue: Future<_i3.SurveyResultEntity>.value(
              _FakeSurveyResultEntity_1())) as _i6
      .Future<_i3.SurveyResultEntity>);
  @override
  _i6.Future<void> validate(String? surveyId) =>
      (super.noSuchMethod(Invocation.method(#validate, [surveyId]),
          returnValue: Future<void>.value(),
          returnValueForMissingStub: Future<void>.value()) as _i6.Future<void>);
  @override
  _i6.Future<void> save(_i3.SurveyResultEntity? surveyResult) =>
      (super.noSuchMethod(Invocation.method(#save, [surveyResult]),
          returnValue: Future<void>.value(),
          returnValueForMissingStub: Future<void>.value()) as _i6.Future<void>);
}

/// A class which mocks [RemoteLoadSurveyResult].
///
/// See the documentation for Mockito's code generation for more information.
class MockRemoteLoadSurveyResult extends _i1.Mock
    implements _i7.RemoteLoadSurveyResult {
  MockRemoteLoadSurveyResult() {
    _i1.throwOnMissingStub(this);
  }

  @override
  String get url =>
      (super.noSuchMethod(Invocation.getter(#url), returnValue: '') as String);
  @override
  _i4.HttpClient get httpClient =>
      (super.noSuchMethod(Invocation.getter(#httpClient),
          returnValue: _FakeHttpClient_2()) as _i4.HttpClient);
  @override
  _i6.Future<_i3.SurveyResultEntity> loadBySurvey({String? surveyId}) => (super
      .noSuchMethod(Invocation.method(#loadBySurvey, [], {#surveyId: surveyId}),
          returnValue: Future<_i3.SurveyResultEntity>.value(
              _FakeSurveyResultEntity_1())) as _i6
      .Future<_i3.SurveyResultEntity>);
}
